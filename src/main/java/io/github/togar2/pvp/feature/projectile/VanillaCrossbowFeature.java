...
public class VanillaCrossbowFeature implements CrossbowFeature, RegistrableFeature {
	...
	private static final Tag<Boolean> START_SOUND_PLAYED = Tag.Transient("minestompvp:crossbow_sound_start");
	private static final Tag<Boolean> MID_LOAD_SOUND_PLAYED = Tag.Transient("minestompvp:crossbow_sound_mid");
	...
	public void init(EventNode<EntityInstanceEvent> node) {
		...
		node.addListener(PlayerTickEvent.class, event -> {
			...
			boolean playStartSound = Boolean.FALSE.equals(player.getTag(START_SOUND_PLAYED));
			boolean playMidLoadSound = Boolean.FALSE.equals(player.getTag(MID_LOAD_SOUND_PLAYED));

			if (progress >= 0.2 && playStartSound) {
			...
			if (progress >= 0.5F && midLoadSound != null && playMidLoadSound) {
				ViewUtil.viewersAndSelf(player).playSound(Sound.sound(
