...
public class VanillaDamageFeature implements DamageFeature, RegistrableFeature {
	...
	public static final Tag<Long> NEW_DAMAGE_TIME =
		Tag.<Long>Transient("minestompvp:new_damage_timestamp")
			.defaultValue(-10_000L);

	public static final Tag<Float> LAST_DAMAGE_AMOUNT =
		Tag.<Float>Transient("minestompvp:last_damage_amount")
			.defaultValue(0.0f);
	...
	protected void handleDamage(EntityDamageEvent event) {
		...
		long newDamageTime = entity.getTag(NEW_DAMAGE_TIME);
		if (entity.getAliveTicks() - newDamageTime < 0) {
			float lastDamage = entity.getTag(LAST_DAMAGE_AMOUNT);
